...
export async function getNegocis() {
  try {
    return await fetchSheet('Negocis', 86400); // 24h
  } catch (e) {
...
export async function getGuies() {
  try {
    return await fetchSheet('Guies', 3600); // 1h
  } catch (e) {
...
export async function getAgenda() {
  try {
    const items = await fetchSheet('Agenda', 3600); // 1h
    return items.map(e => ({
      id:        String(e.id || ''),
      titol:     e.titol || '',
      data:      e.data || '',
    ...
    console.error('Error sheets agenda:', err);
    return [];
  }
}
